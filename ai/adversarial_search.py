...
class TicTacToe(Game):
    ...
    def __init__(self, h=3, v=3, k=3):
        update(self, h=h, v=v, k=k) ## utils.py function
...
class ConnectFour(Game):
    ...
    def __init__(self, h=6, w=7, k=4):
        update(self, h=h, w=w, k=k)  ## utils.py function
        moves = [(x, y) for x in range(1, h + 1)
                 for y in range(1, w + 1)]
        self.initial = Struct(to_move='R', utility=0, board={}, moves=moves)
    ...
    def make_move(self, move, state):
        ...
        moves = list(state.moves)
        moves.remove(move)
        return Struct(to_move=if_(state.to_move == 'R', 'Y', 'R'),
                      utility=self.compute_utility(board, move, state.to_move),
                      board=board, moves=moves)
